...
Vorteile: - Hilft eigenes CSS zu strukturieren - Die Komponenten bilden Namens-Basis

Nachteile: - Je HTML-Element nur eine Klasse erlaub - kann zu langen Klassennamen führe - geringe Spezifität
...
`Control` + `Command` + `Space` für Icons in CodePen.io
...
Normalerweise gibt man dem Html keine eigene Farbe.

skope?

`opacity: 0.n` Sichtbarkeit

Laufzeitverkürzung in CSS mittels `Type`
